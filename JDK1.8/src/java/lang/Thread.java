 ...
 * <p>
 * Every thread has a priority(优先次序). Threads with higher priority are
 ...
 * called and the security manager has permitted(被允许的) the exit operation
 * to take place.
 * <li>All threads that are not daemon threads have died, either by
 * returning from the call to the <code>run</code> method or by
 * throwing an exception that propagates(增值) beyond the <code>run</code>
 * method.
 * </ul>
 * <p>
...
public class Thread implements Runnable {
    ...
    /* Whether or not to single_step(单步执行) this thread. */
    ...
    /* ThreadLocal values pertaining(附属) to this thread. This map is maintained
     ...
     * Accessed(存取) using java.util.concurrent.locks.LockSupport.getBlocker
     */
